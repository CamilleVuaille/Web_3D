...
class EsiTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testAddSurrogateControl()
    {
        ...
        $response = new Response('foo <esi:include model="" />');
    ...
    public function testRenderIncludeTag()
    {
        ...
        $this->assertEquals('<esi:include model="/" onerror="continue" alt="/alt" />', $esi->renderIncludeTag('/', '/alt', true));
        $this->assertEquals('<esi:include model="/" alt="/alt" />', $esi->renderIncludeTag('/', '/alt', false));
        $this->assertEquals('<esi:include model="/" onerror="continue" />', $esi->renderIncludeTag('/'));
        $this->assertEquals('<esi:comment text="some comment" />'."\n".'<esi:include model="/" onerror="continue" alt="/alt" />', $esi->renderIncludeTag('/', '/alt', true, 'some comment'));
    }
    ...
    public function testProcess()
    {
        ...
        $request = Request::create('/');
        $response = new Response('foo <esi:comment text="some comment" /><esi:include model="..." alt="alt" onerror="continue" />');
        $esi->process($request, $response);

        $this->assertEquals('foo <?php echo $this->surrogate->handle($this, \'...\', \'alt\', true) ?>'."\n", $response->getContent());
        $this->assertEquals('ESI', $response->headers->get('x-body-eval'));

        $response = new Response('foo <esi:comment text="some comment" /><esi:include model="foo\'" alt="bar\'" onerror="continue" />');
        $esi->process($request, $response);
        ...
        $response = new Response('foo <esi:include model="..."></esi:include>');
        $esi->process($request, $response);
