...
class Esi implements SurrogateInterface
{
    ...
    {
        $html = sprintf('<esi:include model="%s"%s%s />',
            $uri,
    ...
    public function process(Request $request, Response $response)
    {
        ...
        while (isset($chunks[$i])) {
            $options = array();
            preg_match_all('/(model|onerror|alt)="([^"]*?)"/', $chunks[$i], $matches, PREG_SET_ORDER);
            foreach ($matches as $set) {
                $options[$set[1]] = $set[2];
            }

            if (!isset($options['model'])) {
                throw new \RuntimeException('Unable to process an ESI tag without a "model" attribute.');
            }

            $chunks[$i] = sprintf('<?php echo $this->surrogate->handle($this, %s, %s, %s) ?>'."\n",
                var_export($options['model'], true),
